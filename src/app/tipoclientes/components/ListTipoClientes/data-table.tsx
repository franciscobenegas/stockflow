...
export function DataTableTipoCliente({ data }: DataTableProps) {
  ...
  const [deletingCliente, setDeletingCliente] = useState<tipoCliente | null>(
    null
  );
  ...
  const handleDeleteConfirm = async () => {
    if (deletingCliente) {
      ...
      try {
        const resp = await fetch(`/api/tipocliente/${deletingCliente.id}`, {
          method: "DELETE",
          headers: {
  ...
  const columns: ColumnDef<tipoCliente>[] = [
    {
      ...
      cell: ({ cell }) => {
        return <div className="ml-4 ">{cell.getValue() as string}</div>;
      },
    },
